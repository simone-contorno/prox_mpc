...
- **One controller, many vehicles:** the identical plugin drives a unicycle and a bicycle by configuration alone, re-confirmed 40/40 in this pass (mode b1), with no model-specific result separating the two - the one figure that appeared to single out the bicycle proved to be a first-cycle transient both models produce identically, sampled on some runs and not others (Sections 6.3, 8).
