...
- **The tail is heavy on the hard cells, and the worst case now overruns the budget.** On the dynamic and multi-obstacle cells ProxMPC's **p95 rises to 5.7-7.0 ms** (predictive 4.0-5.6 ms), against DWB's and MPPI's steadier 3.0-3.7 ms: with many half-plane rows simultaneously active the worst-case QP is heavier than a fixed-size sampler. Every p95 still sits ~7x inside the 50 ms control budget, but **5 runs of 427 recorded a single cycle above 50 ms, peaking at 321 ms** (four of them predictive, on the multi and circle cells). This is a direct consequence of rebuilding the QP factorization every cycle rather than reusing it (a cross-cycle warm start is prototyped on the `feat/qp-warm-start` branch; it does not apply to this increment formulation - see [QP warm starting across control cycles](../README.md#qp-warm-starting-across-control-cycles)). ProxMPC is much cheaper on average but far more variable; the sampling controllers are steadier.
