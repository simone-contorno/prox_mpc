...
Mode (b1), the ground-truth predictive results in Section 6.3, was re-run separately at commit `1e170ad` on the same host, against the same core and controller binaries (no source file changed between the two commits).
Mode (a), the Gazebo results in Section 7, was **not** re-run in this pass and still reflects the `v1.0.0` baseline; it is called out again where it appears.
...
| Run mode | **(b2)** Nav2 stack on a kinematic plant, no Gazebo (see [run modes](prox-mpc.md#9-running-the-stack-the-three-modes)), at commit `988db0a`; mode (b1) for the ground-truth predictive results in Section 6.3, re-run at commit `1e170ad` on the same host and the same binaries |
...
One controller-vs-tracker separation is worth recording: fed the *correct* future obstacle positions (mode b1, the engine knows the exact trajectory, orbit included), ProxMPC still reaches the goal on every single moving-obstacle case for both vehicle models.
Section 6.2 called for exactly this check after reactive ProxMPC began colliding 3/5 on the same orbiting obstacle, and it was re-run for this pass:
...
| static_box | Unicycle / Bicycle | 5/5 / 5/5 | 0.000 / 0.000 | 4.43 / 4.54 | 0.490 / 1.963 |
...
The single-obstacle dynamic case therefore remains fully within the controller's reach given a correct predictor, and the reactive orbit failure in Section 6.2 is a limit of what the costmap supplies, not of the formulation.
...
Two measured shifts from the `v1.0.0` b1 table are worth recording, neither of them attributed to a specific change in the intervening work:

- **`static_box`/bicycle breached the harness's own slack threshold on 3 of 5 runs**, at a peak keep-out relaxation of 2.945 m against the `max_obstacle_slack_m: {max: 0.5}` bound in [`prox_mpc_benchmark/config/metrics.yaml`](../prox_mpc_benchmark/config/metrics.yaml); the other 2 runs sat at 0.490 m, as did all five unicycle runs on the same cell. The `v1.0.0` table and both earlier recorded b1 campaigns report a flat ~0.49 m with zero spread across all five repeats, so this bimodal relaxation is new and is not run-to-run measurement noise. It changes none of the success, geometry, feasibility, or deadline results above, and it is the one b1 result this pass would not call clean.
...
- **The mode-b1 re-run left one result this pass would not call clean, and one unexplained uniform shift** (Section 6.3). On `static_box`/bicycle the peak keep-out slack was bimodal across five repeats - 2.945 m on three runs, 0.490 m on two - and the 2.945 m figure breaches the harness's own `max_obstacle_slack_m: {max: 0.5}` threshold; both earlier recorded b1 campaigns and the `v1.0.0` table show a flat ~0.49 m with zero spread, so this is new rather than measurement noise, and no mechanism in the intervening diff is traced to it. Separately, b1 solve time rose on all eight cells (median +0.4 to +2.7 ms) with no candidate cause in the diff, since mode b1 uses neither the costmap nor the Nav2 stack; it is reported as observed. Neither affects the b1 success, geometry, feasibility, or deadline results, which are unchanged in kind from the `v1.0.0` table.
...
- **One controller, many vehicles:** the identical plugin drives a unicycle and a bicycle by configuration alone, re-confirmed 40/40 in this pass (mode b1). The one blemish is on the bicycle: 3 of 5 `static_box` runs relaxed the soft keep-out to 2.945 m, past the harness's own 0.5 m threshold, with no counterpart in any earlier b1 campaign (Sections 6.3, 8).
