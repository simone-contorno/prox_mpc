...
**What it is.**
The scenario-driven harness that *measures* the stack across three metric classes — **accuracy** (cross-track / goal error), **precision** (mean ± std over repeats), and **real-time / feasibility** (solver diagnostics) — for a matrix of *scenario × model × controller × run mode*.
