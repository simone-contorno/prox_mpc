...
| `prox_mpc_benchmark` | Metrics node + Python tooling + kinematic plant | Measures accuracy / precision / real-time across the scenario × model × controller × mode matrix; hosts the mode (b2) Nav2 plant. See [prox_mpc_benchmark/README.md](../prox_mpc_benchmark/README.md) and the [comparison results](controller-comparison-results.md). |
