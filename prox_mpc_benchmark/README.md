...
- **Result summary (matched 0.5 m/s cap; reactive ProxMPC at `safety_margin: 0.2`,
  ...
  **+0.157 m** (RPP +0.124, DWB +0.098, MPPI +0.002, Graceful -0.006, Vector
  Pursuit -0.020), while reactive ProxMPC is the narrowest at -0.062 m. Its
...
*The eleven **`proxmpc`** rows were measured on 2026-08-27 at commit `40d1e61` (branch `test/audit-benchmark-revalidation`), with the reactive preset's `safety_margin` at **0.2 m** (keep-out `d_safe = robot_radius + safety_margin = 0.42 m`) - 11 scenarios x 5 repeats = 55 runs, from `results/mode_b2_margin020_20260827_074543/scenarios.json`.*

*The rows for the other six controllers - `dwb`, `mppi`, `regulated_pure_pursuit`, `graceful`, `vector_pursuit`, and **`proxmpc_pred`** (which pins `safety_margin: 0.1`) - carry over unchanged from the 2026-08-26 campaign at commit `988db0a` (branch `chore/audit-warning-hardening`), 380 runs, on the same host. They were deliberately not re-run: none of their presets reads the changed controller default, and the physical obstacle geometry the harness draws comes from a fixed harness constant, so it is identical in both sessions.*
...
> b1 and b2 numbers above are unaffected (neither uses Gazebo), and both were
> re-measured on this host in separate campaigns, described above.
