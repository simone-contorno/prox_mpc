...
*Mode `b2` re-validated on the post-audit-chain tree at commit `988db0a` (branch `chore/audit-warning-hardening`); auto-generated by `aggregate.py` from `results/mode_b2_revalidation_20260825_235827/scenarios.json` (435 runs). Mode `b1` re-validated separately at commit `1e170ad` (branch `test/audit-benchmark-revalidation`), auto-generated by `aggregate.py` from `results/mode_b1_revalidation_20260826_155128/scenarios.json` (40 runs) - same host, and no source file changed between the two commits, so both tables measure the same core and controller binaries. 475 runs in total. Values are mean±std (population) over repeats - the precision signal. Not committed.*
