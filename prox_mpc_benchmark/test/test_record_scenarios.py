...
def test_rviz_runs_at_the_lowest_priority_without_offload():
    cmd = rs.rviz_command(Path('recording.rviz'), False)
...
def test_gpu_offload_moves_only_rviz_to_the_nvidia_gpu():
    cmd = rs.rviz_command(Path('recording.rviz'), True)
    assert cmd[0] == 'env'
    ...
    assert cmd[cmd.index('nice'):] == rs.rviz_command(Path('recording.rviz'), False)
