...
The in-loop keep-out disc is still centred on the model's reference point and
sized `robot_radius + safety_margin`. For a model referenced away from
`base_link` that disc does not cover the whole robot on its own; the footprint
veto, now checked at the correct pose, is what covers the rest.
...
`ProxMpcController` gains one protected method and eight protected members, which
changes `sizeof(ProxMpcController)`. The pluginlib load path is unaffected,
